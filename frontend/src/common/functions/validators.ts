...
export namespace Validators {
  ...
  export function aboveEntropy(value: number) {
    return (controler: AbstractControl) => {
      ...
      const calculateEntrophy = (phrase: string): number => {
        const charsCounter = Array.from(phrase).reduce((p, c) => {
          if (!p[c]) {
            p[c] = 0;
          }
          p[c]++;
        ...
        const pswdLen = phrase.length;
        return Object.values<number>(charsCounter).reduce((p, c) => p - c / pswdLen * Math.log2(c / pswdLen), 0);
      };

      const calculatePartMaxEntrophy = (part: number, phrase: string): number => {
